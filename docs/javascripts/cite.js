...
(function () {
  ...
  async function fetchText(path) {
    ...
    if (!resp.ok)
      throw new Error(`[cite] Failed to fetch ${url}: ${resp.status}`);
    return resp.text();
  }
  ...
  function cslJsonToRis(cslData) {
    var records = cslData.map(function (entry) {
      ...
      if (entry["container-title"])
        lines.push("T2  - " + entry["container-title"]);
      ...
      if (entry.page) {
        ...
        if (pages.length > 1)
          lines.push("EP  - " + pages[pages.length - 1].trim());
      }
      ...
      if (entry["publisher-place"])
        lines.push("CY  - " + entry["publisher-place"]);
  ...
  function buildFormatTabs(
    ...
    formats.forEach(function (fmt, idx) {
      ...
      if (fmt.id === "bibtex") {
        ...
        try {
          var cslData = keys
            .map(function (key) {
              return cite.data.find(function (d) {
                return d.id === key;
              });
            })
            .filter(Boolean);
      ...
      if (fmt.id === "ris") {
        ...
        contentsHtml +=
          ...
          '<button class="cite-copy-btn" data-copy-text="' +
          escapedText +
          '" title="Copy to clipboard">' +
          ICON_COPY +
          " Copy</button>" +
          '<button class="cite-save-btn" data-save-text="' +
          escapedText +
          '" data-filename="' +
          downloadFilename +
          '" title="Download RIS file">' +
          ICON_SAVE +
          " Download .ris</button>" +
          "</div>" +
          "</div>";
        ...
        contentsHtml +=
          ...
          '<pre class="cite-format-pre"><code>' +
          escapedText +
          "</code></pre>" +
          '<button class="cite-copy-btn cite-copy-inline" data-copy-text="' +
          escapedText +
          '" title="Copy to clipboard">' +
          ICON_COPY +
          "</button>" +
    ...
    var shouldExpand = keys.some(function (k) {
      return AUTO_EXPAND_KEYS.has(k);
    });

    var chevron =
      '<span class="cite-chevron"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></span>';

    return (
      '<details class="cite-formats"' +
      (shouldExpand ? " open" : "") +
      ">" +
      "<summary>Copy/Export Citation" +
      chevron +
      "</summary>" +
   ...
   */
  async function buildCiteWidget(
    widgetEl,
    cite,
    availableKeys,
    bibtexEntries,
    ...
    var html =
      '<details class="cite-widget-details" open>' +
    ...
    if (locationEntries.length > 0) {
      html +=
        '<div class="cite-widget-locations">' +
      ...
      locationEntries.forEach(function (loc) {
        ...
        html +=
          '<label class="cite-widget-checkbox-label">' +
          '<input type="checkbox" data-prefix="' +
          loc.prefix +
          '"' +
          checked +
          "> " +
      ...
      html +=
        '<p class="cite-widget-hint">A dataset-level citation is included automatically. ' +
  ...
  function initCiteTooltips(container, keyToTooltip) {
    ...
    tooltip.innerHTML =
      '<div class="cite-tooltip-bridge"></div><div class="cite-tooltip-body"></div>';
  ...
  async function processCitations() {
    ...
    var keyToNum = {};
    var keyToTooltip = {};
    var validCitedKeys = citedKeys.filter(function (k) {
      return availableKeys.has(k);
    });
    validCitedKeys.forEach(function (key, i) {
      keyToNum[key] = i + 1;
    });
    ...
    content.innerHTML = content.innerHTML.replace(
      ...
      function (fullMatch, inner) {
        ...
        const vKeys = keys.filter(function (k) {
          return availableKeys.has(k);
        });
        ...
        if (vKeys.length === 0) {
          ...
          return (
            '<span class="cite-error" title="Unknown citation key">' +
            fullMatch +
            "</span>"
          );
        }
        ...
        try {
          ...
          var mainCitation =
            '<div class="cite-full">' + parts.join("") + "</div>";
          var formatTabs = buildFormatTabs(
            cite,
            vKeys,
            bibtexEntries,
            extraTemplatesLoaded,
      ...
      function (fullMatch, inner) {
        ...
        try {
          ...
          return (
            '<a href="#cite-ref-' +
            firstKey +
            '" class="cite-inline cite-link" data-cite-key="' +
            firstKey +
            '">' +
            inlineCite +
            "</a>"
    ...
    if (validCitedKeys.length > 0) {
      try {
        ...
        var bibHtml =
          '<ol class="cite-bib-list">' + listItems.join("\n") + "</ol>";
        ...
        if (bibDivs.length > 0) {
          ...
          var contentInner =
            content.querySelector(".md-content__inner") || content;
          if (widget) {
